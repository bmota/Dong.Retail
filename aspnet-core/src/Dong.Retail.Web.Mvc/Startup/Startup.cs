...
using Dong.Retail.Identity;
using Dong.Retail.MultiTenancy;
using Dong.Retail.Web.Authentication.JwtBearer;
using Dong.Retail.Web.Resources;
using PaulMiami.AspNetCore.Mvc.Recaptcha;
using Swashbuckle.AspNetCore.Swagger;
using Dong.Retail.Web.IdentityServer;

#if FEATURE_SIGNALR
using Owin;
using Abp.Owin;
using Dong.Retail.Web.Owin;
#endif
...
namespace Dong.Retail.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            //{
            //    options.SwaggerDoc("v1", new Info { Title = "Retail API", Version = "v1" });
            ...
            return services.AddAbp<RetailWebMvcModule>(options =>
            {
        ...
        {
            app.Properties["host.AppName"] = "Retail";

            app.UseAbp();

            app.MapSignalR();

            //Enable it to use HangFire dashboard (uncomment only if it's enabled in RetailWebCoreModule)
            //app.UseHangfireDashboard("/hangfire", new DashboardOptions
            //{
            //    Authorization = new[] { new AbpHangfireAuthorizationFilter(AppPermissions.Pages_Administration_HangfireDashboard) }
            //});
